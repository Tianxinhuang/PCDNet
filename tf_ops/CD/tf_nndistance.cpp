...
static void nnsearch(int b,int n,int m,int dnum,const float * xyz1,const float * xyz2,float * dist,int * idx){
	float c2list[16];
	for (int i=0;i<b;i++){
		for (int j=0;j<n;j++){
			...
			for (int k=0;k<m;k++){
				...
				double d=0;
                                for (int dd=0;dd<dnum;dd++){
                                        d=d+(c2list[dd]-xyz2[(i*m+k)*dnum+dd])*(c2list[dd]-xyz2[(i*m+k)*dnum+dd]);
                                }
				if (k==0 || d<best){
					best=d;
					besti=k;
...
class NnDistanceOp : public OpKernel{
	public:
		...
		void Compute(OpKernelContext * context)override{
			...
			int dnum=xyz1_tensor.shape().dim_size(2);
			...
			nnsearch(b,n,m,dnum,xyz1,xyz2,dist1,idx1);
			nnsearch(b,m,n,dnum,xyz2,xyz1,dist2,idx2);
		}
};
...
class NnDistanceGradOp : public OpKernel{
	public:
		...
		void Compute(OpKernelContext * context)override{
			...
			OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,n,dnum},&grad_xyz1_tensor));
			Tensor * grad_xyz2_tensor=NULL;
			OP_REQUIRES_OK(context,context->allocate_output(1,TensorShape{b,m,dnum},&grad_xyz2_tensor));
			...
			for (int i=0;i<b;i++){
				for (int j=0;j<n;j++){
					...
					for (int d=0;d<dnum;d++){
						int j2=idx1[i*n+j];
						float g=grad_dist1[i*n+j]*2;
						grad_xyz1[(i*n+j)*dnum+d]+=g*(xyz1[(i*n+j)*dnum+d]-xyz2[(i*m+j2)*dnum+d]);
						grad_xyz2[(i*m+j2)*dnum+d]-=(g*(xyz1[(i*n+j)*dnum+d]-xyz2[(i*m+j2)*dnum+d]));
					}
				}
				for (int j=0;j<m;j++){
					...
					for (int d=0;d<dnum;d++){
                                                int j2=idx2[i*m+j];
                                                float g=grad_dist2[i*m+j]*2;
                                                grad_xyz1[(i*m+j)*dnum+d]+=g*(xyz1[(i*m+j)*dnum+d]-xyz2[(i*n+j2)*dnum+d]);
                                                grad_xyz2[(i*n+j2)*dnum+d]-=(g*(xyz1[(i*m+j)*dnum+d]-xyz2[(i*n+j2)*dnum+d]));
                                        }
				}
			}
		}
...
void NmDistanceKernelLauncher(int b,int n,int dnum,const float * xyz,int m,const float * xyz2,float * result,int * result_i,float * result2,int * result2_i);
class NnDistanceGpuOp : public OpKernel{
	public:
		...
		void Compute(OpKernelContext * context)override{
			...
			float * dist2=&(dist2_flat(0));
			int * idx2=&(idx2_flat(0));
			NmDistanceKernelLauncher(b,dnum,n,xyz1,m,xyz2,dist1,idx1,dist2,idx2);
		}
};
REGISTER_KERNEL_BUILDER(Name("NnDistance").Device(DEVICE_GPU), NnDistanceGpuOp);

void NmDistanceGradKernelLauncher(int b,int n,int dnum,const float * xyz1,int m,const float * xyz2,const float * grad_dist1,const int * idx1,const float * grad_dist2,const int * idx2,float * grad_xyz1,float * grad_xyz2);
class NnDistanceGradGpuOp : public OpKernel{
	public:
		...
		void Compute(OpKernelContext * context)override{
			...
			auto grad_xyz2_flat=grad_xyz2_tensor->flat<float>();
			float * grad_xyz2=&grad_xyz2_flat(0);
			NmDistanceGradKernelLauncher(b,dnum,n,xyz1,m,xyz2,grad_dist1,idx1,grad_dist2,idx2,grad_xyz1,grad_xyz2);
		}
};
