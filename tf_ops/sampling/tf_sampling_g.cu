...
__global__ void farthestpointsamplingKernel(int b,int n,int m,int dnum, const float * __restrict__ dataset,float * __restrict__ temp,int * __restrict__ idxs){
  if (m<=0)
    return;
  ...
  __shared__ float buf[BufferSize*256];
  float clist1[256];
  float clist2[256];
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    int old=0;
    ...
    for (int j=threadIdx.x;j<min(BufferSize,n)*dnum;j+=blockDim.x){
      buf[j]=dataset[i*n*dnum+j];
    }
    __syncthreads();
    for (int j=1;j<m;j++){
      int besti=0;
      ...
      for (int dd=0;dd<dnum;dd++){
          clist1[dd]=dataset[i*n*dnum+old*dnum+dd];
      }
	  ...
	  for (int dd=0;dd<dnum;dd++){
              clist2[dd]=dataset[i*n*dnum+k*dnum+dd];
          }
        }
	float d=0;
        for (int dd=0;dd<dnum;dd++){
              d=d+(clist1[dd]-clist2[dd])*(clist1[dd]-clist2[dd]);
          }
        //float d=(x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)+(z2-z1)*(z2-z1);
	//float d=min(min((x2-x1)*(x2-x1),(y2-y1)*(y2-y1)),(z2-z1)*(z2-z1));
        float d2=min(d,td);
        if (d2!=td)
...
void farthestpointsamplingLauncher(int b,int n,int m,int dnum,const float * inp,float * temp,int * out){
  farthestpointsamplingKernel<<<32,512>>>(b,n,m,dnum,inp,temp,out);
}
