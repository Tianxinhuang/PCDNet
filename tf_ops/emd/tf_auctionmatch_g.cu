#include <cstdio>
__global__ void AuctionMatchKernel(int b,int n, int dnum,const float * __restrict__ xyz1,const float * __restrict__ xyz2,int * matchl,int * matchr,float * cost){
	//this kernel handles up to 4096 points
	const int NMax=4096;
	//int dnum=sizeof(*xyz1)/(b*n*sizeof(xyz1[0]));
	...
	for (int bno=blockIdx.x;bno<b;bno+=gridDim.x){
		...
		for (int k0=0;k0<n;k0+=Block){
			...
			for (int k=threadIdx.x;k<(k1-k0)*dnum;k+=blockDim.x)
				buf[k]=xyz1[bno*n*dnum+k0*dnum+k];
			__syncthreads();
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
				...
				for (int dd=0;dd<dnum;dd++){
				        c2list[dd]=xyz2[bno*n*dnum+j*dnum+dd];
				}
				for (int k=k0;k<k1;k++){
					...
					float d=0;
					for (int dd=0;dd<dnum;dd++){
                                                d=d+(c2list[dd]-buf[(k-k0)*dnum+dd])*(c2list[dd]-buf[(k-k0)*dnum+dd]);
					}
					d=sqrtf(d);
...
void AuctionMatchLauncher(int b,int n,int dnum,const float * xyz1,const float * xyz2,int * matchl,int * matchr,float * cost){
	AuctionMatchKernel<<<32,512>>>(b,n,dnum,xyz1,xyz2,matchl,matchr,cost);
}
